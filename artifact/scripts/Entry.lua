...
uevr.sdk.callbacks.on_pre_engine_tick(
    function(engine, delta)
        ...
        if gameState:IsLevelChanged(engine) then
            ...
            if gameState.isConversation and engineTickCount % 10 == 0 then
                 uevr.params.vr.set_mod_value("VR_AimMethod", "0")
            end
            ...
            if weaponChanged or scopeChanged then
                ...
                if currentWeaponMesh ~= nil then
                    ...
                    uevrUtils.setTimeout(500, function()
                        ...
                        if capWeaponMesh == gameState:GetEquippedWeapon() and capWeaponName and not gameState.isMontageAttached and not gameState.inMenu and not isWeaponModMontageActive and not isMagazineMontageActive then
                            local pawn = gameState:GetLocalPawn()
...
uevr.lua.add_script_panel("Stalker 2 VR", function()
    ...
    if imgui.collapsing_header("Gestures Toggles") then
        ...
        local inventoryChanged, newInventory = imgui.checkbox("Inventory Backpack (left shoulder using left hand)", Config.gestures.inventory)
    ...
    if imgui.collapsing_header("Debug Dev Config") then
        ...
        if imgui.tree_node("Scope Settings") then
            ...
            local dChanged, nD = imgui.drag_float("Scope Offset (X)", Config.cylinderDepth, 0.001, 0.0, 0.5, "%.3f")
