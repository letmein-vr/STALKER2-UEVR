...
local bodyMesh = nil
local levelLoadDelay = 0  -- ticks to skip initDecoupledYaw after a level load
...
uevr.sdk.callbacks.on_pre_engine_tick(function(engine, delta)
	...
	-- Decrement the post-level-load delay. While it is non-zero, skip
	-- initDecoupledYaw() so that transient spawn animations (e.g. exo equip)
	-- cannot bake a wrong yaw into the movement axis on the first tick.
	if levelLoadDelay > 0 then
		levelLoadDelay = levelLoadDelay - 1
	end
...
uevrUtils.registerPreLevelChangeCallback(function(level)
	...
	bodyMesh = nil
	cameraComponent:reset()
	-- Delay decoupledYaw re-init for 60 ticks after load so the exo spawn
	-- animation settles before we bake in the pawn's yaw as the movement axis.
	levelLoadDelay = 60
end)
