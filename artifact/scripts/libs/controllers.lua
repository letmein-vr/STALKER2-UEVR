	...
	for i = 0, 1 do
		if getCachedController(i) == nil then
			...
			if controller ~= nil then
				...
				-- Guard: GetOwner() can return nil when the owning actor has been
				-- garbage collected during a script restart or level transition.
				-- Calling get_full_name() on nil crashes with "attempt to index a nil value".
				local owner = controller:GetOwner()
				local ownerName = owner and owner:get_full_name() or "(owner nil)"
