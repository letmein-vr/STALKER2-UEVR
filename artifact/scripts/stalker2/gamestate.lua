...
-- Uses two instant signals instead of the old 50-tick world-time-freeze method:
--   1. UIManagerEx.OpenViews contains W_PauseMenuMainView_C  → pause menu
--   2. No active player pawn                                 → main menu / loading screen
-- Confirmed via live UEVR API inspection:
--   Pause menu  → OpenViews: [W_PauseMenuMainView_C], pawn: exists
--   Main menu   → OpenViews: [],                     pawn: nil
--   Loading     → OpenViews: [],                     pawn: nil
function GameStateManager:CheckMenuState()
    -- 1. Pause menu: W_PauseMenuMainView_C appears in UIManagerEx.OpenViews
    ...
    if uiMgr then
        ...
        if openViews then
            for _, view in ipairs(openViews) do
                ...
                if ok and cls == "W_PauseMenuMainView_C" then
                    self.inMenu = true
                    return
                end
            end
...
-- Check if player is climbing or mantling
-- Runs every tick for immediacy.
--
-- isClimbing = any Mode 5/6 event (ladder, mantle, topaz) — preserves all existing camera/aim logic
-- isMantling = Mode 5 where bAnimClimbStarted==false (mantle/topaz) — used for arm suppression
--
-- MCP confirmed: during ladder, bAnimClimbStarted becomes true on the SAME tick as Mode 5.
--               during mantle, bAnimClimbStarted is NEVER true throughout Mode 5.
function GameStateManager:CheckClimbingState()
    local prevClimbing = self.isClimbing
    local prevMantling = self.isMantling

    self.isClimbing = false
    self.isMantling = false

    local pawn = self:GetLocalPawn()
    if pawn then
        local charMovement = pawn.CharacterMovement
        if charMovement then
            local mode = charMovement.MovementMode
            if mode == 5 or mode == 6 then
                -- isClimbing: true for ANY Mode 5 event (restores camera/aim/attachment logic)
                self.isClimbing = true

                -- isMantling: true ONLY when bAnimClimbStarted is false (mantle, topaz)
                -- When bAnimClimbStarted is true → real ladder → isMantling stays false
                local bAnimClimbStarted = false
                if pawn.Mesh and pawn.Mesh.AnimScriptInstance then
                    local cd = pawn.Mesh.AnimScriptInstance["ClimbingData"]
                    if cd then
                        bAnimClimbStarted = (cd["bAnimClimbStarted"] == true)
                    end
                end
                if not bAnimClimbStarted then
                    self.isMantling = true  -- Mantle or topaz: suppress pawn arms
                end
            end
        end
    end

    if prevClimbing ~= self.isClimbing then
    end
    if prevMantling ~= self.isMantling then
    end
end
...
function GameStateManager:get_scope_mesh(parent_mesh)
    ...
    -- Bug D fix: also scans grandchildren to catch scopes mounted on rails/adapters.
    local fallback = nil

    local function check_component(component)
    ...
    for _, component in ipairs(child_components) do
        local best = check_component(component)
        if best then return best end
        -- Check grandchildren
        if component.AttachChildren then
            for _, gc in ipairs(component.AttachChildren) do
                local gc_best = check_component(gc)
                if gc_best then return gc_best end
            end
        end
    end
    return fallback
