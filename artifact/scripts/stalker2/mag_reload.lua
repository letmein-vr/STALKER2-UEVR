...
function M.update_weapon_collision(pawn, weapon_mesh, boxCfg)
    ...
    if not utils.validate_object(MagBox) then
        local ok, err = pcall(function()
...
-- Called on level change: nil stale component references so M.init() creates fresh
-- boxes on the new pawn. Also resets grip-edge detection and cooldown so the first
-- gesture after the load is not spuriously blocked.
function M.reset()
    BoxCompLH      = nil
    MagBox         = nil
    wasGripHeld    = false
